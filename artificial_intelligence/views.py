...
def run_tsp(request):
    ...
    if(request.method == 'POST'):
        ...
        if n > 0:
            ...
            print "El estado es ESTOOOOOOOOOOOOO",request.POST.get('state')
            start = timeit.default_timer()
            if request.POST.get('state') == 'Backtracking':
              print "Entro en backtracking"
              diff = get_expanded_nodes()
              print "La diferencia fue de: ",diff
              solution = tsp_backtracking(A, 0, 0, matrix.distance_matrix)
              expanded_nodes-=diff
            elif request.POST.get('state')== 'Heuristic':
              print "Entro en heuristic"
            ...
            print "nodes:", nodes
            #opt2 = other_two_otp(get_shortest(), matrix.distance_matrix)
            #print "la optimizacion es:", opt2
            backtracking_solution = Bactracking_Solution(matrix_size=matrix_size, solution_cost=solution_cost, coordinates=coordinates, expanded_nodes=int(expanded_nodes),execution_time=execution_time, nodes=nodes)
            backtracking_solution.save()
            data = {"coordinates_django":coordinates,
                    "matrix_size":n,
